...
export const SellerHomePage = () => {
  ...
  const [isModalOpen, setIsModalOpen] = useState(false); // open modal pop up for modification
  ...
  const [item, addItem] = useState({ // for adding items
    item_name: "",
    item_condition: "",
    size: "",
    description: "",
    price: "",
    item_photo: "",
    member_id: memberId // link memberid with item
  });
  ...
  const handleUpdate = async () => {
    try {
      await axios.put(`http://localhost:8800/items/${selectedItem.item_id}`, selectedItem); // update item
